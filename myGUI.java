...
public class myGUI extends JFrame {
    ...
    private JPanel MainPanel, jpUserInput, panelButtons, outputPanel;
    private JLabel jlEnterFile, tblName, jlConsole, nameLbl, idLbl, genderLbl, ageLbl, raceLbl, tradeLbl;;
    ...
    // saves data from colonyData into colony9 table
    private void saveData() {
        try{
            PreparedStatement pdst = con.prepareStatement("INSERT INTO colony9 VALUES (?, ?, ?, ?, ?, ?)");

            pdst.setInt(1, tableId);
            pdst.setString(2, name);
            pdst.setString(3, gender);
            pdst.setInt(4, age);
            pdst.setString(5,race);
            pdst.setString(6, trade);
...
// Function that reads the user's file, saves to colony9 table, displays table to GUI
        jtfUserFile.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent evt) {
                try{

                    // scan user file, delimit it
                    String file = jtfUserFile.getText();
                    ...
                    while(scnr.hasNextLine()) {
                        ...
                        //Process line for each data item
                        try(Scanner data = new Scanner(line)) {
                            data.useDelimiter(",");

                            // scan data to attributes
                            if(data.hasNextInt()) {
                                tableId = data.nextInt();
                            }
                            if(data.hasNext()) {
                                name = data.next();
                            }
                        ...
                        saveData();
                        selectAll();
                        jtfTblMessage.setText("File successfully inputted");
                    }
                    //end while
                }
                 catch (FileNotFoundException e) {
                    jtfTblMessage.setText("File not found");
